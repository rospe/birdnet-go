...
func TestSpeciesListAPI(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v2/species-lists", nil)
	rec = httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v2/species-lists/1", nil)
	rec = httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	req = httptest.NewRequest(http.MethodDelete, "/api/v2/species-lists/1", nil)
	rec = httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	...
	req = httptest.NewRequest(http.MethodDelete, "/api/v2/species-lists/999", nil)
	rec = httptest.NewRecorder()
	e.ServeHTTP(rec, req)
	require.Equal(t, http.StatusForbidden, rec.Code)
}
